...
func TestMergeBaseEnv(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:     "base-only keys carry through, caller keys win and come last",
			baseEnv:  []string{"PATH=/base/bin", "NVIDIA_VISIBLE_DEVICES=all"},
			env:      []string{"PATH=/override/bin", "FOO=bar"},
			expected: []string{"NVIDIA_VISIBLE_DEVICES=all", "PATH=/override/bin", "FOO=bar"},
		},
		{
		...
		},
		{
			...
			expected: []string{"PATH=/base/bin", "MALFORMED"},
		},
	} {
